...
def adversarial_test(left_df, right_df, cat_cols):
    ...
    train_score, val_score, avg_num_trees, model = lgb_model.fit(
...
def model_metrics(y_test, prediction,dataset_name,encoder,trainshape,validation_type,sample_type):
    accuracy = accuracy_score(y_test, prediction)
    recall = recall_score(y_test, prediction)
    precision = precision_score(y_test, prediction)
    roc_auc = roc_auc_score(y_test, prediction)
    f1score = f1_score(y_test, prediction)
    kappa_metric = cohen_kappa_score(y_test, prediction)
    gmean = geometric_mean_score(y_test, prediction, pos_label=1)
    df = pd.DataFrame({
